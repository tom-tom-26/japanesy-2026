...
- `scripts/html_common.py` — dependency-free renderers (`render_event`, `render_day`, `render_pill`, `render_section`) that turn the data above back into the exact `<details class="ev">` / `<details class="card daycard">` markup the `<style>` in `japan_holiday_updated.html` expects. Also `render_month_calendar()` — builds the Overview tab's November-2026 grid from both cities' `days`/`pills` (labels derive from the pills, so they stay in sync automatically).
- `scripts/template_shell.html` — the full page shell (hero, nav, Overview, Flights, `<style>`, `<script>`, footer) with `{{KYOTO_*}}` / `{{TOKYO_*}}` / `{{MONTH_CAL}}` markers where the generated day-cards and Overview calendar splice in. Hand-edit this directly for anything that isn't a Kyoto/Tokyo day-card (new tab, CSS tweak, script change).
- `WEBSITE_STRUCTURE.md` — **read this before editing the site.** Deep-dive on the generator pipeline: exact data schema (day/event/pill fields), which file to touch for a given change, copy-paste edit recipes, the month-calendar design, devtools verification snippets, and the known gotchas (two `2026-11-12` day cards, HTML-fragment values needing manual escaping). Written to keep site edits cheap.
